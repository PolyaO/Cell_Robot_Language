#include "backend/rvals/binary.hpp"

#include <stdexcept>

#include "backend/rvals.hpp"
#include "backend/rvals/rval.hpp"
#include "interpreter/exceptions/runtime_exceptions.hpp"
#include "interpreter/exec_ctx.hpp"

ast::Binary::Binary(unsigned idx1, unsigned idx2, Binary::Op op, unsigned line)
    : _idx1(idx1), _idx2(idx2), _op(std::move(op)), _line(line) {}

var::var_type ast::Binary::execute(ExecCtx &ctx) const {
    try {
        return _op(ast::execute(ctx, ctx.ast.get_rval(_idx1)),
                   ast::execute(ctx, ctx.ast.get_rval(_idx2)));
    } catch (const std::runtime_error &e) {
        throw InterpreterRuntimeError(std::format("{} {}", _line, e.what()));
    }
}
unsigned ast::Binary::get_line() { return _line; }
