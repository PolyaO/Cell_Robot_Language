...
ast::Extend::Extend(unsigned idx, unsigned dim, unsigned change, unsigned line)
    : _idx(idx), _dim(dim), _change(change), _line(line) {}

var::var_type ast::Extend::execute(ExecCtx &ctx) const {
    try {
        return var::extend(ast::execute(ctx, ctx.ast.get_rval(_idx)), _dim,
                           _change);
    } catch (const std::runtime_error &e) {
        throw InterpreterRuntimeError(std::format("{} {}", _line, e.what()));
    }
}
unsigned ast::Extend::get_line() { return _line; }
