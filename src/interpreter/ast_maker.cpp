...
void ast::AstMaker::check_for_taskname_redeclaration(
    ...
    if (task_idx) {
        ...
        throw TaskRedeclare(task_name, line, loc.begin.line);
    }
}
...
ast::AstMaker::var_dict_t::const_iterator ast::AstMaker::check_for_var_unknown(
    ...
    auto it = find_var(var_name);
    if (it == _variables_avaliable.end())
        throw VarUnknown(var_name, loc.begin.line);
    return it;
}
...
void ast::AstMaker::add_to_arg_list(
    ...
    if (std::holds_alternative<unsigned>(list)) {
        ...
        if (it == _variables_avaliable.end())
            throw ArgNameUnknown(arg, loc.begin.line);
...
unsigned ast::AstMaker::make_task(
    ...
    check_for_taskname_redeclaration(task_name);
    auto it = find_var(res);
    if (it == _variables_avaliable.end())
        throw ResNameUnknown(res, task_name, loc.begin.line);
    auto task_idx =
        _ast.make_expr<Task>(std::move(exprs), std::get<1>(*it),
                             std::get<unsigned>(arg_list), loc.begin.line);
...
unsigned ast::AstMaker::make_findexit(std::vector<unsigned> &&exprs) {
    check_for_taskname_redeclaration("FINDEXIT");
    auto task_idx =
        _ast.make_expr<Task>(std::move(exprs), 0, 0, loc.begin.line);
...
unsigned ast::AstMaker::make_switch(unsigned rval_idx, bool condition1,
                                    ...
                                    unsigned stmt2) {
    if (condition1) {
        if (condition2) throw DoubleLogicLiteral("TRUE", loc.begin.line);
        return _ast.make_expr<Switch>(rval_idx, stmt1, stmt2, loc.begin.line);
    }
    if (!condition2 && stmt2) throw DoubleLogicLiteral("FALSE", loc.begin.line);
    return _ast.make_expr<Switch>(rval_idx, stmt2, stmt1, loc.begin.line);
}

unsigned ast::AstMaker::make_scope(std::vector<unsigned> &&exprs) {
    remove_scope_variables(exprs);
    return _ast.make_expr<Scope>(std::move(exprs), loc.begin.line);
}
