...
int main(void) {
    ...
    file_w.open("../winutil/src/winutil/window-file-view.cpp");
    file_w.select({87, 0}, {88, 0});
    ...
    while (kbhit()) {
        // clang-format off
        switch (getchar()) {
        case 'k': file_w.scroll(-1); break;
        case 'j': file_w.scroll(1); break;
        case 'a': file_w.scroll_to(87, true); break;
        case EOF: goto end;
        }
        // clang-format on
