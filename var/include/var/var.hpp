...
Var<T>::const_iterator Var<T>::end() const {
    return std::visit([](auto &v) { return (const T *)v.data() + v.size(); },
                      _val);
}
...
template <class T>
var_type Var<T>::copy() const {
    dim_t dim = _dim;
    return std::visit(
        [&](auto &v) {
            return Var<T>(std::move(dim), std::vector<T>(v.begin(), v.end()));
        },
        _val);
}
